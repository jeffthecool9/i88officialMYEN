...
const stats = [
  { value: '3X', label: 'Top Up RM100 · Get RM300' },
  { value: '20%', label: 'Daily · Max RM288/Day' },
  { value: 'RM5K', label: 'Weekly Pass Prize' },
]
...
export default function Hero() {
  const handleHeroCTAClick = () => {
    ...
    trackEvent('english_hero_cta_click', {
      section: 'hero',
      button_text: 'Join Now',
      destination_url: DESTINATION_URL,
    })
  ...
  return (
    <section className="relative min-h-[100dvh] flex flex-col overflow-hidden">
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse 70% 45% at 20% 82%, rgba(14,165,233,0.16) 0%, transparent 70%)',
        }}
      />
        ...
        >
          ...
          >
            <span
              ...
              style={{
                fontSize: 'clamp(1.5rem, 6.5vw, 3.2rem)',
            ...
            <span
              ...
              style={{
                fontSize: 'clamp(0.7rem, 3vw, 1.25rem)',
          ...
          <motion.p
            variants={rise}
            className="font-body text-body-premium text-sm leading-relaxed mb-4 md:max-w-md"
          >
          ...
          >
            {stats.map((stat, index) => (
              <div
                key={stat.value}
                className={`stat-cell text-center py-2.5 px-4 ${
                  index < stats.length - 1
                    ? 'border-r border-white/8'
                    : ''
                }`}
              >
                <p className="font-display text-stat-value text-xl leading-none tracking-wide">
                  {stat.value}
                </p>

                <p className="font-body text-stat-label text-[10px] tracking-widest uppercase mt-1">
                  {stat.label}
                </p>
              </div>
            ))}
